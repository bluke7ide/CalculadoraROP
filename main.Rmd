...
```{r}
x <- 65
sim <- sapply(1:1000, function(x) renta_permanente(c(1e6,2e6,5e6), 65, 38e6, 0, intereses(65))$pension)
...
fig <- ggplot() +
  geom_line(data = df_long,
            aes(x = Tiempo, y = Pension/1e3, group = Simulacion, color = "Simulaciones"),
  ...
  geom_line(data = df_res_lineas, 
            ...
            linewidth = 1) +
  geom_hline(aes(yintercept = 30638.35/1e3, color = "Monto mínimo"),
